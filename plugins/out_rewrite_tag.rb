...
module Fluent
  class RewriteTagOutput < Output
    Plugin.register_output('rewrite_tag', self)
    ...
    def configure(conf)
      ...
      conf.keys.select{|k| k =~ /^rewriterule(\d+)$/}.sort_by{|i| i.sub('rewriterule', '').to_i}.each do |key|
        ...
        if regexp.nil? || rewritetag.nil?
          raise ConfigError, "failed to parse rewriterules at #{key} #{conf[key]}"
        end

        unless rewritetag.match(/\$\{tag_parts\[\d\.\.\.?\d\]\}/).nil? or rewritetag.match(/__TAG_PARTS\[\d\.\.\.?\d\]__/).nil?
          raise ConfigError, "${tag_parts[n]} and __TAG_PARTS[n]__ placeholder does not support range specify at #{key} #{conf[key]}"
        end
      ...
      unless @rewriterules.length > 0
        raise ConfigError, "missing rewriterules"
      end

      unless @rewriterules.length == rewriterule_names.uniq.length
        raise ConfigError, "duplicated rewriterules found #{@rewriterules.inspect}"
      end
    ...
    def emit(tag, es, chain)
      es.each do |time,record|
        rewritten_tag = rewrite_tag(tag, record)
        next if rewritten_tag.nil? || tag == rewritten_tag
